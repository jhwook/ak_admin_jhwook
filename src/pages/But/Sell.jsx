...
export const Sell = () => {
  ...
  useEffect(() => {
    axios.get(api.API_TRANSATION + `/${page * rows}/${rows}/id/DESC`, {
      params: { filterkey: 'typestr', filterval: 'SWAP'}
    }).then((res) => {
      console.log("res", res);
            ...
            <tbody>
              {tableData.map((item, index) => (
                ...
                >
                  ...
                  <td>@ioisdfsfsdgsg</td>
                  <td>{JSON.parse(item.auxdata).to_token_symbol}</td>
                  <td>{JSON.parse(item.auxdata).to_amount ? parseFloat(JSON.parse(item.auxdata).to_amount).toFixed(6) : '-'}</td>
                  <td>진행중</td>
                  <td>{JSON.parse(item.auxdata).fee_amount ?`${Math.abs(parseFloat(JSON.parse(item.auxdata).fee_amount).toFixed(6))} AKD` : '-'}</td>
                  <td> {JSON.parse(item.auxdata).fee_amount ?`${parseFloat(JSON.parse(item.auxdata).from_amount).toFixed(6)} AKD` : '-'}</td>
